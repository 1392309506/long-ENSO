...
# BCC-CSM2-MR
stage2_ckpt_dir=./output/train_stage2/exp10
stage2_config=$stage2_ckpt_dir/config.json

batch_size=8
base_out=./output/predict/exp10
...
for lead in $(seq 1 21); do
  ...
  echo "Running lead_time=${lead}, output=${out_dir}"

  CUDA_VISIBLE_DEVICES=$gpu python -u predict.py \
    --save_preds True \
    --save_vars 'tos' \
    --ckpt_list $stage2_ckpt_dir \
    --config_path_list $stage2_config \
    --dist_port $dist_port \
    --data_dir $data_dir \
...
wait
echo "All lead-time jobs finished."
